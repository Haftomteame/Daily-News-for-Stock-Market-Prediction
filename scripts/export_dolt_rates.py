#!/usr/bin/env python3
"""Exporte chaque table du depot Dolt rates vers CSV en parallele."""
...
def _repo_dir(dolt_root: Path) -> Path:
    ...
    if not repo_dir_valid(repo_dir):
        raise FileNotFoundError(
            f"Depot Dolt rates introuvable : {repo_dir} (pas de .dolt/)"
        )
    return repo_dir
...
def _collect_jobs(
    ...
    *,
    ...
    if tables:
        ...
        if missing:
            raise ValueError(f"Tables absentes dans rates : {', '.join(missing)}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Exporte les tables du depot Dolt rates vers CSV en parallele.",
    )
    parser.add_argument(
        "--dolt-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Dossier de sortie (defaut: <dolt-root>/exports/rates)",
    )
    parser.add_argument(
        "--tables",
        nargs="+",
    ...
    if not jobs:
        print("Aucune table a exporter dans rates.")
        return 0

    if args.dry_run:
        print(f"[dry-run] {len(jobs)} export(s) rates -> {output_dir}\n")
        for job in jobs:
    ...
    print(
        f"Export de {len(jobs)} table(s) rates vers {output_dir} "
