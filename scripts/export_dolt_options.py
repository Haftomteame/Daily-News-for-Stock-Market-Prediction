#!/usr/bin/env python3
"""Exporte chaque table du depot Dolt options vers CSV en parallele."""
...
def _repo_dir(dolt_root: Path) -> Path:
    ...
    if not repo_dir_valid(repo_dir):
        raise FileNotFoundError(
            f"Depot Dolt options introuvable : {repo_dir} (pas de .dolt/)"
        )
    return repo_dir
...
def _collect_jobs(
    ...
    *,
    ...
    if tables:
        ...
        if missing:
            raise ValueError(f"Tables absentes dans options : {', '.join(missing)}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Exporte les tables du depot Dolt options vers CSV en parallele.",
    )
    parser.add_argument(
        "--dolt-root",
        type=Path,
        default=Path("/data"),
        help="Dossier contenant le clone options/ (defaut: /data dans ubuntu-box)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Dossier de sortie (defaut: <dolt-root>/exports/options)",
    )
    parser.add_argument(
        "--tables",
        nargs="+",
        metavar="TABLE",
        help="Limiter a certaines tables du depot options.",
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    if not jobs:
        print("Aucune table a exporter dans options.")
        return 0

    if args.dry_run:
        print(f"[dry-run] {len(jobs)} export(s) options -> {output_dir}\n")
        for job in jobs:
    ...
    print(
        f"Export de {len(jobs)} table(s) options vers {output_dir} "
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for future in as_completed(futures):
            ...
            if result.error:
                failed += 1
                print(
                    f"  [ERREUR] {result.table} : {result.error}",
                    file=sys.stderr,
                    flush=True,
                )
