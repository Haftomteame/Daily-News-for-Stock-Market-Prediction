#!/usr/bin/env python3
"""Importe le depot Dolt options vers PostgreSQL (schema: options)."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Importe le depot Dolt options vers PostgreSQL (schema options).",
    )
    parser.add_argument(
        "--dolt-root",
        type=Path,
    ...
    if args.dry_run:
        ...
        print(f"[dry-run] options -> schema {schema} : {len(tables)} table(s)")
    ...
    print(f"PostgreSQL OK — import options depuis {dolt_root}\n")
