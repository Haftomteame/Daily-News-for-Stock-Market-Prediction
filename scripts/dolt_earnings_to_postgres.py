#!/usr/bin/env python3
"""Importe le depot Dolt earnings vers PostgreSQL (schema: earnings)."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Importe le depot Dolt earnings vers PostgreSQL (schema earnings).",
    )
    parser.add_argument(
        "--dolt-root",
        type=Path,
    ...
    if args.dry_run:
        ...
        print(f"[dry-run] earnings -> schema {schema} : {len(tables)} table(s)")
    ...
    print(f"PostgreSQL OK — import earnings depuis {dolt_root}\n")
